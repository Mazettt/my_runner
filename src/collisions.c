...
int collisions(beginning_t *begin, all_objects_t *all_objs, scoreboard_t *score)
{
    ...
    panda_rocher[0].top += 15;
    panda_rocher[0].left += 15;
    panda_rocher[0].height -= 30;
    panda_rocher[0].width -= 30;
    for (int i = 0; i < NBR_OBSTACLES; ++i) {
        panda_rocher[1] =
        sfSprite_getGlobalBounds(all_objs->obstacles[i].sprite);
        panda_rocher[1].top += 15;
        panda_rocher[1].left += 15;
        panda_rocher[1].height -= 30;
        panda_rocher[1].width -= 30;
