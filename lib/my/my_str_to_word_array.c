...
char **strtowordarray2(char const *str, int var_word, char **new_str, int k)
{
    for (int all_letters = 0; str[all_letters] != '\0'; ++all_letters) {
        ...
        if (my_is_alphanum(str, all_letters) == 1) {
            ...
            new_str[var_word] = malloc(sizeof(char) * (size + 1));
            for (k = 0; k < size; ++k) {
                new_str[var_word][k] = str[all_letters];
                ++all_letters;
            }
            new_str[var_word][k] = '\0';
            ++var_word;
        }
    }
    new_str[var_word] = NULL;
    return (new_str);
}
...
char **my_str_to_word_array(char const *str)
{
    ...
    new_str = strtowordarray2(str, var_word, new_str, k);
    return (new_str);
}
